...
interface JoinAWorkspaceProps {
  onGoBack: () => void;
  onJoinWorkspace: (workspaceCode: string, userName: string) => void;
  onLogoClick: () => void;
  initialCode?: string;
}

export default function JoinAWorkspace({ onGoBack, onJoinWorkspace, onLogoClick, initialCode = '' }: JoinAWorkspaceProps) {
  const [workspaceCode, setWorkspaceCode] = useState(initialCode);
  const [userName, setUserName] = useState('');
  
  // Update workspace code if initialCode changes
  React.useEffect(() => {
    if (initialCode) {
      setWorkspaceCode(initialCode);
    }
  }, [initialCode]);

  const handleSubmit = () => {
    if (workspaceCode.trim() && userName.trim()) {
      onJoinWorkspace(workspaceCode.trim(), userName.trim());
    }
  };

  return (
    <div className="bg-[#e9e6df] relative size-full flex items-center justify-center" data-name="Join a workspace">
        ...
        <div className="content-stretch flex flex-col gap-[24px] items-center w-[264px]">
          <div className="content-stretch flex flex-col gap-[40px] items-start relative shrink-0 w-full">
            <div className="content-stretch flex flex-col gap-[16px] items-center relative shrink-0 w-full">
              <p className="font-['Inter_Display:Regular',sans-serif] leading-[normal] not-italic relative shrink-0 text-[18px] text-center text-stone-500 tracking-[-0.18px] w-[240px]">join a workspace by pasting the code below</p>
              <div className="bg-stone-50 relative rounded-[8px] shrink-0 w-full" data-name="Input">
                <div className="content-stretch flex items-center overflow-clip relative rounded-[inherit] w-full">
                  <input
                    type="text"
                    placeholder="enter a workspace code"
                    value={workspaceCode}
                ...
                <div aria-hidden="true" className={`absolute border border-solid ${workspaceCode.trim() ? 'border-stone-800' : 'border-stone-400'} inset-0 pointer-events-none rounded-[8px] shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] transition-colors`} />
              </div>
            </div>
              ...
              <div className="basis-0 bg-stone-500 hover:bg-stone-600 active:bg-stone-700 transition-colors grow min-h-px min-w-px relative rounded-[8px] shrink-0 cursor-pointer" data-name="_button" onClick={handleSubmit}>
                <div className="flex flex-row items-center justify-center overflow-clip rounded-[inherit] size-full">
                  <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[16px] py-[14px] relative w-full">
                    <p className="font-['Inter_Display:SemiBold',sans-serif] leading-[normal] not-italic relative shrink-0 text-[15px] text-center text-nowrap text-stone-50 whitespace-pre">join a workspace</p>
                  </div>
                </div>
              </div>
            </div>
